...
class captcha(commands.Cog):
    ...
    async def verify(self, ctx):
        ...
        if(rescode==200):
            ...
            return await ctx.send(f'{rescode} 오류가 발생했습니다! 오류 내용: {self.error[int(rescode)]}')
        ...
        if(rescode==200):
            response_body = response.read()
            with open(f'{author.id}.jpg', 'wb') as f:
                value = f.write(response_body)
        else:
            print("Error Code:" + rescode)
            return
        code = "1"
        ...
        try:
            ...
            return await ctx.send('> 캡차가 취소 되었습니다!')
        ...
        if(rescode==200):
            ...
            print("Error Code:" + rescode)
            return await ctx.send('> API 오류가 났습니다! 잠시만 기달려 주세요!')
